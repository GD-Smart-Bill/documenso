...
test('[TEAMS]: send team email request', async ({ page }) => {
  ...
  await page.getByPlaceholder('eg. Legal').fill('test@test.smartbill.co.il');
  await page.getByPlaceholder('example@example.com').click();
  await page.getByPlaceholder('example@example.com').fill('test@test.smartbill.co.il');
  ...
  await expect(
    page
      .getByRole('status')
      .filter({
        hasText: 'We have sent a confirmation email for verification.',
      })
      .first(),
  ).toBeVisible();
});
...
test('[TEAMS]: accept team email request', async ({ page }) => {
  ...
  const teamEmailVerification = await seedTeamEmailVerification({
    email: `team-email-verification--${team.url}@test.smartbill.co.il`,
    teamId: team.id,
  });
