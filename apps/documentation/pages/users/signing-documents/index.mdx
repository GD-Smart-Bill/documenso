...
The guide assumes you have a Smartsign account. If you don't, you can create a free account [here](https://documen.so/free-docs).

### Upload Document

Navigate to the [Smartsign dashboard](https://sign.smartbill.co.il/documents) and click on the "Add a document" button. Select the document you want to upload and wait for the upload to complete.

![Smartsign dashboard](/document-signing/documenso-documents-dashboard.webp)
...
![Smartsign document overview](/document-signing/documenso-uploaded-document.webp)
...
![Advanced settings for a document uploaded in the Smartsign dashboard](/document-signing/documenso-uploaded-document-advanced-options.webp)
...
Smartsign enables you to set up access control for your documents. You can require authentication for viewing the document.
...
![Document access settings in the Smartsign dashboard](/document-signing/documenso-enterprise-document-access.webp)
...
![Document recipient action authentication in the Smartsign dashboard](/document-signing/document-enterprise-recipient-action-authentication.webp)
...
Smartsign has 4 roles for recipients with different permissions and actions.
...
Smartsign supports 9 different field types that can be added to the document. Each field type collects various information from the recipients when they sign the document.

![The available field types in the Smartsign dashboard](/document-signing/documenso-document-fields.webp)
...
![Email settings in the Smartsign dashboard](/document-signing/documenso-document-email-settings.webp)

If you leave the email subject and message empty, Smartsign will use the default email template.
...
![How to copy the signing link for each recipient from the Smartsign dashboard](/document-signing/documenso-signing-links.webp)

The signing link has the following format:

```bash
https://sign.smartbill.co.il/sign/12ACP777zxQLO52hjj_vCB
```
