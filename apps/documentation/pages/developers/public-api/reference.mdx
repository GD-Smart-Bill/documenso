---
title: API Reference
description: Reference documentation for the Smartsign public API.
---
...
The Swagger UI for the API is available at [/api/v1/openapi](https://sign.smartbill.co.il/api/v1/openapi). This page provides detailed information about the API endpoints, request and response formats, and authentication requirements.

## Upload a Document

Uploading a document to your Smartsign account requires a two-step process.
...
- `authOptions` - This object contains authentication options for the document. It has the following fields:
  - `globalAccessAuth` - The authentication level required to access the document. This can be either `ACCOUNT` or `null`.
    - If the document is set to `ACCOUNT`, all recipients must authenticate with their Smartsign account to access it.
    - The document can be accessed without a Smartsign account if it's set to `null`.
  - `globalActionAuth` - The authentication level required to perform actions on the document. This can be `ACCOUNT`, `PASSKEY`, `TWO_FACTOR_AUTH`, or `null`.
    - If the document is set to `ACCOUNT`, all recipients must authenticate with their Smartsign account to perform actions on the document.
...
The upload URL is a pre-signed S3 URL that you can use to upload the document to the Smartsign (or your) S3 bucket. You need to make a `PUT` request to this URL to upload the document.
  ...
  "recipients": [
    {
      ...
      "role": "SIGNER",
      "signingOrder": 1,
      "signingUrl": "https://sign.smartbill.co.il/sign/<unique-signer-token>"
    },
    {
      "recipientId": 12,
      "name": "Ash Drew",
...
Once the document is successfully uploaded, you can access it in your Smartsign account dashboard. The screenshot below shows the document that was uploaded via the API.
...
Smartsign allows you to generate documents from templates. This is useful when you have a standard document format you want to reuse.
...
The first step is to retrieve the template ID from the Smartsign dashboard. You can find the template ID in the URL by navigating to the template details page.
  ...
  "recipients": [
    {
      ...
      "role": "SIGNER",
      "signingOrder": null,
      "signingUrl": "https://sign.smartbill.co.il/sign/<signing-token>"
    }
  ]
}
```

You can now access the document in your Smartsign account dashboard. The screenshot below shows the document that was generated from the template.
...
![A screenshot of the document in the Smartsign editor](/api-reference/fields-added-via-api.webp)
...
Check out the endpoint in the [API V1 documentation](https://sign.smartbill.co.il/api/v1/openapi#:~:text=/%7BtemplateId%7D/-,generate,-%2Ddocument).
...
Check out the endpoint in the [API V2 documentation](https://openapi.smartbill.co.il/reference#tag/template/POST/template/use).
